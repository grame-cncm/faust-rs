...
`FRS-LEX-*` (1), `FRS-PARSE-*` (3), `FRS-SRC-*` (6), `FRS-EVAL-*` (9),
...
| `FRS-EVAL-0009` | `eval` | `cinput(i, e)` or `coutput(i, e)` with an invalid index `i`: not a compile-time integer (the message names `i` and flags swapped arguments when `e` is a constant), negative, or at or past the number of control inputs (bargraphs) of `e`. faust-rs extension, no C++ equivalent. | `EvalError::ControlIndexNotConstant` and `EvalError::ControlIndexOutOfRange`, raised by `eval_control_entry` (`crates/eval/src/control_inputs.rs`) |
