...
// through ro.interleave (a `route`), and rad differentiates the signal graph
// after propagation, where neither obstacle exists.
//
// Outputs: [y, dy/dw0, dy/dw1, dy/dw2, dy/db], the gradient of the scalar y;
// the body is feed-forward, so the reverse sweep is symbolic and the lanes
// equal the tangents of fad_neuron_sigmoid.dsp sample by sample.
//   dy/dwi = y * (1 - y) * xi
//   dy/db  = y * (1 - y)
...
process = rad((par(i, NW, _), weights : neuron(NW, sigmoid)), weights);
