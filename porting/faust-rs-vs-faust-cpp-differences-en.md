...
| DIFF-CLI-010 | `-mem` / `-mem0` / long aliases | `adapted` | The four C++ mode-zero spellings normalize to one typed per-request `MemoryManagerMode::Mem0` and canonical `-mem0` metadata. The option is intentionally limited to scalar C, C++, and native Cranelift; C and Cranelift are Rust extensions, while `mem1`–`mem3`, vector mode, `-it`, and other backends fail closed. M1 implements parsing/propagation; M2 implements the shared version-2 layout/cost analysis with explicit ABI, allocation roles, `Int64`/`Bool`, corrected D6 branches, and checked counters. Backend storage behavior lands in M3–M5 of the authoritative [mem0 plan](custom-memory-manager-mem0-analysis-and-porting-plan-2026-08-13-en.md). |
...
- The FFI follows the broad Faust factory lifecycle but intentionally omits or
  adapts LLVM-specific target, IR, machine-code, object, memory-manager, and
  foreign-registration families.
