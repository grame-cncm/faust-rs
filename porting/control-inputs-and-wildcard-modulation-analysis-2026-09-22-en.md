...
- **The widget in the UI.** Under a 2→1 modulator that drops the widget, `(!, _)`, the widget's signal reaches nothing. faust-rs keeps the widget in the interface, the reference drops it (measured 2026-09-23): the C++ interface is built from the simplified signal graph, at code generation, so a widget cut by `!`, multiplied by a folded zero, or left in a dead `select2` branch never appears (`ampmodeler.dsp`: 25 controls in C++, 29 in faust-rs, the four dead sliders), where `ui_build.rs` walks the box tree and lists every widget. Not a modulation matter: a pruning of controls whose signal reaches no output, aligned on the reference, is a separate pass of the UI builder that concerns every program, and the natural next parity fix; the rebound program's sliders would then leave the interface as they do in C++. A learner that wants its learned values visible attaches bargraphs, as the Web programs of the projects do.
